...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try (Scanner scan = new Scanner(file)) {
            ...
            content = content.substring(0, content.length()-1); // Remove trailing /n
            System.out.println("The text is:\n" + content);

            int numWords = content.split("[ \n]").length;
            System.out.println("\nWords: " + numWords);

            int numSentences = content.split("[.?!]").length;
            System.out.println("Sentences: " + numSentences);

            int numCharacters = getNumCharacters(content);
            System.out.println("Characters: " + numCharacters);

            int[] syllablesInfo = getNumSyllables(content);
